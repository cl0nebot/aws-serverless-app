...
This is generated by AWS SAM CLI for coverage-mock-app - a golang lambda function that simulates the coverage-app service by sending mock responses for coverage request.  Refer `test.sh` for examples about how to invoke this function.

```bash
.
├── Makefile                    <-- Make to automate build
├── README.md                   <-- This instructions file
├── coverage                    <-- Source code for a lambda function
│   ├── main.go                 <-- Lambda function code
│   └── main_test.go            <-- Unit tests
│   └── eligibility.go          <-- Definition of coverage request and response data
...
In this implementation we use the built-in `go get` and the dependency of this app includes AWS Lambda Go SDK:
...
```shell
GOOS=linux GOARCH=amd64 go build -o coverage/coverage ./coverage
```
or simply
```bash
make build
...
If the previous command ran successfully you should now be able to hit the following local endpoint to invoke your function `http://localhost:3000/coverage`
...
aws cloudformation describe-stacks \
    --stack-name coverage-mock-app --query 'Stacks[].Outputs'
```
